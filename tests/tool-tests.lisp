  ...
  (let* ((registry (make-default-tool-registry))
    ...
    (unwind-protect
         (let* ((conversation (conversation-create configuration
           ...
           (test-assert (= (length (tool-registry-tools registry)) 37)
                        "the default registry exposes the complete initial tool set")
           (test-assert (= (length schemas) 6)
                        "the provider schemas contain six namespaces")
           ...
           (test-assert (string= (json-get (aref schemas 4) "name") "lisp")
                        "the named Lisp namespace follows the workspace tools")
           (test-assert (= (length (json-get (aref schemas 4) "tools")) 12)
           ...
           (test-assert (string= (json-get (aref schemas 5) "name") "self")
                        "the active-image namespace is last")
           (test-assert (= (length (json-get (aref schemas 5) "tools")) 11)
