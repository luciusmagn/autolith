  ...
  (let* ((registry (make-default-tool-registry))
    ...
    (unwind-protect
         (let* ((conversation (conversation-create configuration
           ...
           (test-assert (= (length (tool-registry-tools registry)) 33)
                        "the default registry exposes the complete initial tool set")
           (test-assert (= (length schemas) 5)
                        "the provider schemas contain five namespaces")
           ...
           (test-assert (string= (json-get (aref schemas 2) "name") "memory")
                        "persistent memories have a dedicated namespace")
           (test-assert (= (length (json-get (aref schemas 2) "tools")) 5)
                        "the memory namespace exposes five operations")
           (test-assert (string= (json-get (aref schemas 3) "name") "lisp")
                        "the named Lisp namespace follows the workspace tools")
           (test-assert (= (length (json-get (aref schemas 3) "tools")) 12)
           ...
           (test-assert (string= (json-get (aref schemas 4) "name") "self")
                        "the active-image namespace is last")
           (test-assert (= (length (json-get (aref schemas 4) "tools")) 11)
