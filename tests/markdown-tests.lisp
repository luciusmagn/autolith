...
(defun test-markdown-partial-streaming ()
  "Test overflow commits, retained sources, and continuation completion."
  (let ((renderer (markdown-renderer-create :width 24)))
    (multiple-value-bind (rows tail retained)
      ...
      (test-assert (= (length rows) 1)
                   "overflow commits every completed wrapped row")
      (test-assert (string= (markdown-tests--row-text (first rows))
                            "  alpha beta gamma delta")
                   "committed overflow rows carry the body indent")
      (test-assert (search "epsilon" (markdown-tests--row-text tail))
                   "the tail previews the unfinished remainder")
      (test-assert (string= retained "epsilon")
                   "retained source keeps only unfinished text"))
    (let ((rows (markdown-render-line renderer "epsilon zeta")))
      (test-assert (string= (markdown-tests--row-text (first rows))
                            "  epsilon zeta")
                   "completing an overflowed line continues its indent")))
  (let ((renderer (markdown-renderer-create :width 24)))
    (multiple-value-bind (rows tail retained)
        (markdown-render-partial renderer
                                 "**bold text that goes on and on**")
      (declare (ignore tail))
      (test-assert (find (terminal-span :strong "bold text that goes on")
                         (first rows)
                         :test #'equal)
                   "overflow rows keep their strong styling")
      (test-assert (string= retained "**and on**")
                   "retained source reopens the interrupted strong span")))
  (let ((renderer (markdown-renderer-create :width 40)))
    (markdown-render-line renderer "```")
    (multiple-value-bind (rows tail retained)
