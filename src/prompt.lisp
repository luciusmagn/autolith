...
Your tracked source repository belongs to the user: read it freely, never patch it. fs.write and fs.edit refuse it, and durable self-modifications live as overlay files under the Frob data directory, loaded automatically at every startup. Keep persisted definitions small, readable, and documented, following the style in AGENTS.md at the source root. The source root is ~A. The current workspace is ~A. Preserve existing user work.
